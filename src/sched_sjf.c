...
/* Fuction to sort process for SJF scheduling */
void sort_sjf(struct process *p_list, int list_size)
{
	int time = 0;	/* Keeps track of current time */
	int min_burst_time; 	/* Keeps track of smallest burst time currently */
	int list_counter = 0;

	int min_proc = 1;
	int i = min_proc;

	/* First, sort on their arrival time */
	sort_on_arrival_time(p_list, list_size);

	/* Ensure that the process wuth smallest burst time will be executed next */
	time += p_list[0].arrival_time;	/* Time of first execution */
	while (list_counter < list_size)
	{
		time += p_list[list_counter].burst_time;
		min_burst_time = p_list[min_proc].burst_time;

		while (i < list_size)
		{
			if (time >= p_list[i].arrival_time &&
				p_list[i].burst_time < min_burst_time) // Swap with min_proc
			{
				struct process temp = p_list[min_proc];
				p_list[min_proc] = p_list[i];
				p_list[i] = temp;
			}
			i++;
		}
...
int sched_sjf(struct process *p_list, int list_size)
{
	int list_counter = 0;

	/* Sort on burst time */
	sort_sjf(p_list, list_size);
